...
/* This structure is used to representing a thread which is corresponding to
   ...
   It contains the waited flag, if it is true, the child process is waited by parent.
   by Xiaoqi Cao*/
struct sub_thread {
...
struct list* thread_add_fd(uint32_t, struct file*);
struct list* thread_remove_fd(uint32_t);

/* for system calls*/
bool is_direct_child(int);
